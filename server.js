...
app.post('/agent/writing', x402Middleware(0.003, 'writing-agent', agentWallets.writing, broadcast), async (req, res) => {
  ...
  const report = {
    ...
    style, wordCount: 450, structured: true,
  };
...
const TOOLS = [
  { type: 'function', function: { name: 'hire_research_agent', description: 'Hire the Research Agent to gather stock, news, and macro data. Costs 0.005 USDC.', parameters: { type: 'object', properties: { query: { type: 'string' }, tickers: { type: 'array', items: { type: 'string' } } }, required: ['query', 'tickers'] } } },
  { type: 'function', function: { name: 'hire_writing_agent',  description: 'Hire the Writing Agent to polish raw data into a report. Costs 0.003 USDC.',       parameters: { type: 'object', properties: { rawData: { type: 'object' }, style: { type: 'string', enum: ['professional','concise','detailed'] } }, required: ['rawData'] } } },
  { type: 'function', function: { name: 'buy_stock_data',      description: 'Buy stock data for a ticker. Costs 0.001 USDC.',                                   parameters: { type: 'object', properties: { ticker: { type: 'string' } }, required: ['ticker'] } } },
...
async function executeTool(name, args) {
  switch (name) {
    case 'hire_research_agent':
      return x402Fetch(`${SELF}/agent/research`, ORC_KP, 'orchestrator', { method: 'POST', body: JSON.stringify({ query: args.query, stockTickers: args.tickers }) });
    case 'hire_writing_agent':
      return x402Fetch(`${SELF}/agent/writing`, RESEARCH_KP, 'research-agent', { method: 'POST', body: JSON.stringify({ rawData: args.rawData, style: args.style || 'professional' }) });
    case 'buy_stock_data':
      return x402Fetch(`${SELF}/data/stock/${args.ticker}`, RESEARCH_KP, 'research-agent');
    case 'buy_news_data':
      return x402Fetch(`${SELF}/data/news?q=${encodeURIComponent(args.query)}`, RESEARCH_KP, 'research-agent');
    case 'buy_macro_data':
      return x402Fetch(`${SELF}/data/macro`, RESEARCH_KP, 'research-agent');
    default:
...
async function runOrchestrator(query) {
  ...
  const messages = [
    {
      role: 'system',
      content: `You are the Orchestrator agent in AgentMarket Pro — a live AI agent economy on Stellar.
You coordinate specialist agents, each with their own Stellar wallet:
- Research Agent: gathers financial data (costs 0.005 USDC)
- Writing Agent: polishes raw data into a report (costs 0.003 USDC)
You can also buy data directly: Stock 0.001 USDC, News 0.001 USDC, Macro 0.002 USDC.
Always hire the Writing Agent at the end to produce a polished final report.
Every payment is a real USDC transaction on Stellar testnet.`,
    },
  ...
  while (true) {
    ...
    if (choice.finish_reason === 'stop' || !msg.tool_calls?.length) {
      ...
      broadcast({ type: 'report_ready', report, query });
      return;
    }
...
app.post('/run', async (req, res) => {
  ...
  res.json({ ok: true, message: 'Orchestrator running...' });
